...
1- To view the outputs only (i.e., graphs and rules), i.e., no XAI metrics involved, please see <datasetname>-OutputsOnly.ipynb. This can run on Colab, as it is not CPU-intensive.

2- To run experiments with XAI metrics (such as Fidelity, Simplicity, etc.), run the following:
    a- <datasetname>-Main.ipynb; contains all XAI methods except Anchors. Anchors is CPU-intensive and takes more RAM; therefore, they are separately recorded.
    ...
    d- <datasetname>-stats-reviewer.ipynb; contains statistics requested during the reviewer process.
    e- stats.ipynb; shows the output collected from all <datasetname>-stats-reviewer.ipynb.
    f- package-versions.ipynb; shows package versions.
    g- requirements.txt; as the name indicates, obtained using pip freeze > requirements.txt.

3- Among datasets, the lightest one to run is ESDRPD (as it has the least number of rows, i.e., 16X520), followed by Breast Cancer (30X569), Diabetic retinopathy (19X1,151), and CSection (875X15,409).
